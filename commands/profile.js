...
module.exports = {
 ...
 let user = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.member
        ...
        let roles = user.roles.cache.map(x => x).filter(z => z.name !== "@everyone") || 'none'
      ...
      **•** \`Message's Count:\` **${messagecount}**
      ...
      • ${safe}`)
          ...
          .setTimestamp()
          .setFooter('© Simply Dumb', user.user.avatarURL({dynamic: true}))
  ...
  aliases: ['stats'],
  description: 'See the stats of your friend or someone in the server',
  usage: `/profile <user>`
}